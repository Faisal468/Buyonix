...
const Home = () => {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        // Check if the observer target is visible
    ...
    return (
      <div className="relative bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
        ...
        <Link to={`/shop?product=${product._id}`}>
          <div className="h-48 bg-gray-100 flex items-center justify-center overflow-hidden cursor-pointer">
            <img 
              src={imageUrl} 
              alt={product.name}
        ...
        <div className="p-4">
          <Link to={`/shop?product=${product._id}`}>
            <h3 className="font-semibold text-gray-800 mb-2 line-clamp-2 hover:text-teal-600 transition-colors cursor-pointer">
              {product.name}
            </h3>
          </Link>
          ...
          {product.sellerId?.storeName && (
            <p className="text-xs text-gray-500 mb-2">by {product.sellerId.storeName}</p>
          )}
      ...
      <div className="relative w-full h-[80vh] bg-black -mt-20 pt-20">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 h-full flex flex-col justify-center text-white">
          ...
          <p className="text-lg md:text-xl text-gray-200 max-w-2xl">
            Shop with confidence using AI-powered features: Virtual Try-On,
